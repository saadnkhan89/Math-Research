...
bd=[.25*cos(theta) .25+.25*sin(theta);.25*cos(theta2)-.25 .25*sin(theta2);...
    .25*cos(theta1)+.25 .25*sin(theta1);x_bd -x_bd-.5;-x_bd(1:end-1) -x_bd(1:end-1)-.5];
...
Plt=Plt.*(y>-.5-x & y<0 & (-0.5<=x)&(x<=0))+Plt.*(y>-.5+x & y<0 & (x<=0.5)&(x>0))+...
    Plt.*((x+.25).^2+y.^2<1/16 & x<=-.25 & 0<=y & y<.25)+...
    Plt.*(x.^2+(y-.25).^2<1/16 & (x<0.25)&(x>-0.25) & 0.25<=y & y<.5)+...
    Plt.*((x-.25).^2+y.^2<1/16 & x>=.25 &0<=y&y<=.25)+...
    Plt.*(-0.25<x & x<.25& 0<y & y<.25);
EC{1,k}=Plt;
EC{2,k}=nonzeros(Plt(:));
end
