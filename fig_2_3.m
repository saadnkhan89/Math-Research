%% Solving Poisson PDE on Circle (Figure 2.3)
clc
clear all
...
% Forcing function (right hand side of system)
f=16*pi*cos(4*pi*(Int(:,1).^2+Int(:,2).^2))-...
    64*pi^2*(Int(:,1).^2+Int(:,2).^2).*sin(4*pi*(Int(:,1).^2+Int(:,2).^2));

% Discrete operator (left hand side of system)
A=[(-pi^2*(I(:,1).^2+I(:,2).^2)').*exp(pi*1i*(Int(:,1)*I(:,1)'+Int(:,2)*I(:,2)'));...
     exp(pi*1i*( bd(:,1)*I(:,1)'+bd(:,2)*I(:,2)' ) )];

 M=size(A,1);

% Carrying out Singular Value Decomposition to solve system  
    [U,S,V]=svd(A);
    S=diag(S);
    S=S(S>eps);
    S=1./S;
...
% Arranging the solution on the mesh
