...
for k=1:size(N,2)
    ...
    A=[exp(1i*pi*x*I)*(-pi^2*diag(-N(k):N(k)).^2+1i*pi*diag(-N(k):N(k))-2*eye(2*N(k)+1));exp(1i*pi*[-0.5;.5]*I)];
    ...
    c=V*S*U'*[-3*sin(x)+cos(x);sin(-.5)+exp(-.5);sin(.5)+exp(.5)];

% c=A\[-3*sin(x)+cos(x);sin(-.5)+exp(-.5);sin(.5)+exp(.5)];
...
legend('M=N','M=3N','M=10N','interpreter','latex')
grid on